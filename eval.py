...
from models.ldmic_vae import LDMIC_VAE
...
def visualize_features(IFrameCompressor: nn.Module, filepaths: List[Path], output_dir: Path,
    ...
    with tqdm(total=num_frames) as pbar:
        for i in range(num_frames):
            ...
            # 保存特征图为图像
            # feature_image_path = output_dir / f"LDMIC_feature_maps_{i}"
            save_encoded_features_as_images(features, output_dir, f"LDMIC_feature_maps_{i}")
...
def eval_model(IFrameCompressor:nn.Module, left_filepaths: Path, right_filepaths: Path, **args: Any) -> Dict[str, Any]:
    ...
    with tqdm(total=num_frames) as pbar:
        for i in range(num_frames):
            ...
            #print(features_left)
            save_encoded_features_as_images(features_left, "output/features_img/LDMIC")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    # 测试图像目录
    parser.add_argument("-d", "--dataset", type=str, default='data/ldmic/cityscapes/', help="sequences directory")
    # 测试数据集名称
    ...
    # 模型路径 LDMIC_VAE/lamda4096/train-run3
    parser.add_argument("--i_model_path", type=str, default='checkpoints/cityscapes/mse/LDMIC/lamda64/train-run2/ckpt.pth.tar',help="Path to a checkpoint")
    # 是否裁剪图像
...
def main(args: Any = None) -> None:
    ...
    if args.IFrameModel == "LDMIC":
        IFrameCompressor = LDMIC(N=64, M=64, decode_atten=JointContextTransfer)
    elif args.IFrameModel == "LDMIC_VAE":
        IFrameCompressor = LDMIC_VAE(N=96, M=192, decode_atten=JointContextTransfer)
